...
Future<BLEInfo> _getBLEInfos(String accessToken, String userId) async {
  ...
  if (response.statusCode == 200) {
    return BLEInfo.fromJson(json.decode(response.body));
  } else {
    throw Exception(response.body);
  }
}
    ...
    if (bleInfo.success!) {
      ...
      if (bleInfo.message == "expired") {
        ...
        if (tokenInfo.isUpdated == true) {
          ...
          repeat++;
          if (repeat < 2) {
            return await _processBLEInfos(secureStorage, tokenInfo.getAccessToken(), tokenInfo.getRefreshToken(), userId, repeat);
          } else {
            return BLEInfo(success: false, bleInfoDatas: []);
          }
        }
      }
    }
...
Future<BLEInfo?> sendMessageByBLE(SecureStorage secureStorage) async {
  ...
  if (acccessToken == null) {
    return BLEInfo(success: false, message: Env.MSG_NOT_TOKEN, bleInfoDatas: []);
  }
  ...
  BLEInfo bleInfo = await _processBLEInfos(secureStorage, acccessToken, refreshToken, userId!, 0);
  return bleInfo;
}
